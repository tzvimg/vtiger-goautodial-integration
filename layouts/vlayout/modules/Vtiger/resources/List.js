...
jQuery.Class("Vtiger_List_Js",{
   ...
   getInstance: function(){
	...
	/*
	 * Send data to GOautoDial.
	 */
	massSendRecords : function(massActionUrl){
		var thisInstance = this;
		var listInstance = Vtiger_List_Js.getInstance();
		var validationResult = listInstance.checkListRecordSelected();
		if(validationResult != true){
				...
				function(data) {
					...
					if(data) {
						var callback = function(data) {
							...
							params.onValidationComplete = function(form, valid){
								if(valid){
									thisInstance.sendToGOautodial(form)
								}
								return false;
							}
							jQuery('#chooseList').validationEngine(app.validationEngineOptions);
						}
						app.showModalWindow(data, function(data){														
							if(typeof callback == 'function'){
								callback(data);
	...
	sendToGOautodial : function (form){
		var listInstance = Vtiger_List_Js.getInstance();
		var selectedIds = listInstance.readSelectedIds(true);
		var excludedIds = listInstance.readExcludedIds(true);
		var cvId = listInstance.getCurrentCvId();
		var list = jQuery('#aaa').val();
		...
		AppConnector.request(params).then(
			function(data) {
				if(data.success){
					...
					var params = {
						title : app.vtranslate('JS_MESSAGE'),
						text: app.vtranslate('JS_RECORDS_SENT_TO_GOAUTODIAL'),
						animation: 'show',
						type: 'info'
					};
					Vtiger_Helper_Js.showPnotify(params);
